...
class GodotGooglePlayBilling(godot: Godot): GodotPlugin(godot), PurchasesUpdatedListener {
	...
	override fun getPluginSignals(): MutableSet<SignalInfo> {
        ...
        signals.add(SignalInfo("billing_config_response", Dictionary::class.java))
    ...
    fun getBillingConfig() {
        ...
        billingClient.getBillingConfigAsync(params) { billingResult, billingConfig ->
            val r = Dictionary()
            r["status"] = billingResult.responseCode
            r["debug_message"] = billingResult.debugMessage
            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK && billingConfig != null) {
                r["country_code"] = billingConfig.countryCode
            }
